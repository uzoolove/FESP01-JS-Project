...
const TodoRegist = (): HTMLElement => {
  //NOTE - 페이지 요소 생성
  const page: HTMLElement = document.createElement('div');
  page.setAttribute('id', 'page');
  
  //NOTE - 등록 폼 요소 생성
  const RegistForm: HTMLFormElement = document.createElement('form');
  ...
  const submitRegisterButton: HTMLButtonElement = document.createElement('button');
  const submitRegisterButtonText: Text = document.createTextNode('등록');
